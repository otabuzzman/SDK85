...
KMODE   EQU      0       ; CONTROL CHAR. TO SET KEYBOARD/OISPLAY MODE
...
RES10:                   ; CONTINUE SAVING USER STATUS
        NOP              ; GET USER INTERRUPT STATUS AND INTERRUPT MASK
...
EXAM:
        ...
        JNC     ERR      ; NO - DISPLAY ERROR MSG. AND TERMINATE COMMAND
EXM05:
        ...
        JNC     EXM10    ; NO - DO NOT UPDATE REGISTER CONTENTS
...
EXM10:
        ...
        CALL    NXTRG    ; YES - ADVANCE REGISTER POINTER TO
                         ; /NEXT REGISTER
                         ; ANY MORE REGISTERS ?
        JC      EXM05    ; YES - CONTINUE PROCESSING WITH NEXT REGISTER
...
STP23:
        MVI     A,((TIMER & 0FF00H) / 256) | TMODE ; HIGH ORDER BITS OF TIMER VALUE
                         ; /OR'ED WITH TIMER MODE
        OUT     TIMHI
        MVI     A,TIMER & 0FFH ; LOW ORDER BITS OF TIMER VALUE
        OUT     TIMLO
...
SUB05:
        ...
        JNC     SUB10    ; NO - LEAVE DATA UNCHANGED AT CURRENT ADDRESS
...
;              VALUE AS A 16 BI'C INTEGER. IF MORE THAN 4 HEX DIGITS
...
;              THE BUPFER AS EMPTY AND RETURNS THE CHARACTER
;              AS OUTPUT.
;
RDKBD:
        LXI     H,IBUFF  ; GET INPUT BUPFER ADDRESS
        ...
        JMP     RDKBD
RDK10:
        MVI     M,EMPTY  ; SET BUPFER EMPTY FLAG
...
RGLOC:
        ...
        MVI     H,(RAMST & 0FF00H) / 256 ; GET HIGH ORDER BYTE OF
...
NMTBL:           ; REGISTER NAME TABLE
        ...
        DB      BLANK,LETRP,LETRC,LETRI ; PROGRAM COUNTER LOW BYTE
...
;
RGTBL:
        DB      ASAV     & 0FFH ; A REGISTER
        DB      BSAV     & 0FFH ; B REGISTER
        DB      CSAV     & 0FFH ; C REGISTER
        DB      DSAV     & 0FFH ; D REGISTER
        DB      ESAV     & 0FFH ; E REGISTER
        DB      FSAV     & 0FFH ; FLAGS
        DB      ISAV     & 0FFH ; INTERRUPT MASK
        DB      HSAV     & 0FFH ; H REGISTER
        DB      LSAV     & 0FFH ; L REGISTER
        DB      SPHSV    & 0FFH ; STACK POINTER HIGH ORDER BYTE
        DB      SPLSV    & 0FFH ; STACK POINTER LOW ORDER BYTE
        DB      PCHSV    & 0FFH ; PROGRAM COUNTER HIGH ORDER BYTE
        DB      PCLSV    & 0FFH ; PROGRAM COUNTER LOW ORDER BYTE
...
DCM10:
        ...
        JNC     DCM15    ; IF NOT, MORE TO DISPLAY
...
XCM18:
        ...
        MOV     E,M
        MVI     D,(RAMST & 0FF00H) / 256 ; ADDRESS OF SAVE LOCATION FROM TABLE
        INX     H
...
REG10:
        ...
        MVI     D,(RAMST & 0FF00H) / 256 ; PUT MSP OF SAVE LOC ADDRESS INTO D
...
RTAB:            ; TABLE OF REGISTER INFORMATION
        ...
        DB      ASAV     & 0FFH ; ADDRESS OF REGISTER SAVE LOCATION
